...
class EADEN:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        upper = tf.minimum(tf.subtract(self.slack,self.beta_t), tf.cast(1.0, tf.float32))
        #lower = tf.maximum(tf.add(self.slack,self.beta_t), tf.cast(-0.5, tf.float32))
        lower = tf.maximum(tf.add(self.slack,self.beta_t), tf.cast(0.0, tf.float32)) #change
