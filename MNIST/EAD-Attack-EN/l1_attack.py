...
class EADL1:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        cond1 = tf.cast(tf.greater(tf.subtract(self.slack, self.timg),self.beta_t), tf.float32)  #min{zi-beta,1}, if "zi-x0i>beta"
        cond2 = tf.cast(tf.less_equal(tf.abs(tf.subtract(self.slack,self.timg)),self.beta_t), tf.float32) #x0i, if"|zi-x0i|=<beta"
        cond3 = tf.cast(tf.less(tf.subtract(self.slack, self.timg),tf.negative(self.beta_t)), tf.float32) #max{zi+beta,0},if "zi-x0i<-beta"

        upper = tf.minimum(tf.subtract(self.slack,self.beta_t), tf.cast(1.0, tf.float32)) #min{zi-beta}
        #lower = tf.maximum(tf.add(self.slack,self.beta_t), tf.cast(-0.5, tf.float32))#max{zi+beta}
        lower = tf.maximum(tf.add(self.slack,self.beta_t), tf.cast(0.0, tf.float32))#max{zi+beta} change
        ...
        if self.TARGETED:
            ...
            loss1 = tf.maximum(0.0, other-real+self.CONFIDENCE)   #f(x,t)
        ...
        self.loss = self.loss1+self.loss2+tf.multiply(self.beta_t,self.loss21) #min cf(x,t)+beta*loss1+loss2
