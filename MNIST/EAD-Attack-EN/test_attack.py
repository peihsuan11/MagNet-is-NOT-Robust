...
def main(args):
	with tf.Session() as sess:
		...
		if(args['train']):
			np.save('EN_e-2/labels_train'+str(args['conf'])+'_'+str(args['beta'])+'.npy',labels)
			np.save('EN_e-2/'+str(args['attack'])+'train'+str(args['conf'])+'_'+str(args['beta'])+'.npy',adv)
			np.save('EN_e-2/ID_train'+str(args['conf'])+'_'+str(args['beta'])+'.npy',true_ids)
			return
